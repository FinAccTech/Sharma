...
@AutoUnsubscribe
@Component({
  selector: 'app-deliverydoc',
  templateUrl: './deliverydoc.component.html',
  styleUrls: ['./deliverydoc.component.scss']
})

export class DeliverydocComponent implements OnInit {
  ...
  SelectedDoc: TypeDocHeader = {
    TransSno: 0,
    VouTypeSno: this.globals.VTypDeliveryDoc,
    Series: {SeriesSno:0, Series_Name: "", VouTypeSno: this.globals.VTypDeliveryDoc},
    Trans_No:"",
  ...
  ngOnInit(): void {    
    ...
    if (this.Transaction.TransSno === 0)
      ...
      this.SelectedDoc.VouTypeSno = this.globals.VTypDeliveryDoc;
  ...
  SaveTransaction(){        
    ...
    this.progressService.sendUpdate("start","Saving Delivery Doc");
    trans.saveTransaction().subscribe(data => {
      this.progressService.sendUpdate("stop","");
        if (data.queryStatus == 0) {
          ...
          this.GoBack();                  
          this.globals.SnackBar("info", this.Transaction.TransSno == 0 ? "Buying Contract Created successfully" : "Buying Contract updated successfully",2000);           
        }
    }, 
    error => {
      this.progressService.sendUpdate("stop","");
  ...
  DeleteTransaction(){
    ...
    this.globals.QuestionAlert("Are you sure you wanto to delete this Buying Contract?").subscribe(Response => {      
      if (Response == 1){
