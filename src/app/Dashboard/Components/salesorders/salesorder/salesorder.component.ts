...
export class SalesorderComponent implements OnInit {
  ...
  SelectedDoc: TypeDocHeader = {
    TransSno: 0,
    VouTypeSno: this.globals.VTypSalesOrder,
    Series: {SeriesSno:0, Series_Name: "", VouTypeSno: this.globals.VTypSalesOrder},
    Trans_No:"",
  ...
  ngOnInit(): void {    
    ...
    if (this.Transaction.TransSno === 0)
      ...
      this.SelectedDoc.VouTypeSno = this.globals.VTypSalesOrder;
  ...
  SaveTransaction(){       
    ...
    this.progressService.sendUpdate("start","Saving Sales Order");
    trans.saveTransaction().subscribe(data => {
      this.progressService.sendUpdate("stop","");
        if (data.queryStatus == 0) {
          ...
          this.globals.SnackBar("info", this.Transaction.TransSno == 0 ? "Sales Order Created successfully" : "Sales Order updated successfully",2000);           
  ...
  DeleteTransaction(){
    ...
    this.globals.QuestionAlert("Are you sure you wanto to delete this Sales Order?").subscribe(Response => {      
      if (Response == 1){
