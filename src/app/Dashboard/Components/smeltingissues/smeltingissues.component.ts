...
@Component({
  selector: 'app-smeltingissues',
  templateUrl: './smeltingissues.component.html',
  styleUrls: ['./smeltingissues.component.scss']
}) 

@AutoUnsubscribe
export class SmeltingissuesComponent {
  ...
  ngOnInit(){
    this.LoadSmeltingIssueList(999,999);
  } 

  LoadSmeltingIssueList(FromDate: number, ToDate: number){
    let itm = new ClsTransactions(this.dataService);   
    this.progressService.sendUpdate("start","Loading Smelting Issues..."); 
    itm.getTransactions(0, this.globals.VTypSmeltingIssue,0, FromDate, ToDate).subscribe( data => {  
  ...
  FilterByDate(){
    this.LoadSmeltingIssueList(this.FromDate, this.ToDate) 
  }
  ...
  OpenTransaction(trans: TypeTransactions){            
    trans.VouTypeSno = this.globals.VTypSmeltingIssue;    
    this.router.navigate(['dashboard/smeltingissue', JSON.stringify(trans)]);
  } 
      ...
      if (Response == 1){
        ...
        grp.deleteTransaction().subscribe(data => {
          if (data.queryStatus == 0)
          {
            ...
            this.globals.SnackBar("info","Buying Issue deleted successfully",1500);
