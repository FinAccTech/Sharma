...
@AutoUnsubscribe
@Component({
  selector: 'app-smeltingissue',
  templateUrl: './smeltingissue.component.html',
  styleUrls: ['./smeltingissue.component.scss'] 
})

export class SmeltingissueComponent implements OnInit {
  constructor( private progressService: ProgressbroadcastService, private dataService: DataService, private globals: GlobalsService, private location: Location, private activatedRoute: ActivatedRoute) {
  ...
  SelectedDoc: TypeDocHeader = {
    TransSno: 0,
    VouTypeSno: this.globals.VTypSmeltingIssue,
    Series: {SeriesSno:0, Series_Name: "", VouTypeSno: this.globals.VTypSmeltingIssue},
    Trans_No:"",
  ...
  ngOnInit(): void {    
    ...
    if (this.Transaction.TransSno === 0)
      ...
      this.SelectedDoc.VouTypeSno = this.globals.VTypSmeltingIssue;
  ...
  SaveTransaction(){        
    ...
    this.progressService.sendUpdate("start","Saving Smelting Issue");
    trans.saveTransaction().subscribe(data => {
      this.progressService.sendUpdate("stop","");
        if (data.queryStatus == 0) {
          ...
          this.GoBack();                  
          this.globals.SnackBar("info", this.Transaction.TransSno == 0 ? "Smelting Issue Created successfully" : "Smelting Issue updated successfully",2000);           
        }
    }, 
    error => {
      this.progressService.sendUpdate("stop","");
  ...
  DeleteTransaction(){
    if (this.Transaction.TransSno == 0){
      this.globals.SnackBar("error", "No Issue selected to delete",1200);
      return;
    }
    this.globals.QuestionAlert("Are you sure you wanto to delete this Smelting Issue?").subscribe(Response => {      
      if (Response == 1){
        ...
        itm.deleteTransaction().subscribe(data => {
          if (data.queryStatus == 0)
          {
            ...
            this.globals.SnackBar("info", "Smelting Issue deleted successfully",1500);            
