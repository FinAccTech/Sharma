...
@Component({
  selector: 'app-refiningissues',
  templateUrl: './refiningissues.component.html',
  styleUrls: ['./refiningissues.component.scss']
}) 

@AutoUnsubscribe
export class RefiningissuesComponent {
  ...
  ngOnInit(){
    this.LoadRefiningIssueList(999,999);
  } 

  LoadRefiningIssueList(FromDate: number, ToDate: number){
    let itm = new ClsTransactions(this.dataService);  
    this.progressService.sendUpdate("start","Loading Refining Issues...");  
    itm.getTransactions(0, this.globals.VTypRefiningIssue,0,FromDate, ToDate).subscribe( data => {    
  ...
  FilterByDate(){
    this.LoadRefiningIssueList(this.FromDate, this.ToDate) 
  }
  ...
  OpenTransaction(trans: TypeTransactions){            
    trans.VouTypeSno = this.globals.VTypRefiningIssue;    
    this.router.navigate(['dashboard/refiningissue', JSON.stringify(trans)]);
  } 
  ...
  DeleteItem(trans: TypeTransactions){
    this.globals.QuestionAlert("Are you sure you wanto to delete this Issue?").subscribe(Response => {      
      if (Response == 1){
