...
@AutoUnsubscribe
@Component({
  selector: 'app-refiningissue',
  templateUrl: './refiningissue.component.html',
  styleUrls: ['./refiningissue.component.scss']
})

export class RefiningissueComponent implements OnInit {
  ...
  SelectedDoc: TypeDocHeader = {
    TransSno: 0,
    VouTypeSno: this.globals.VTypRefiningIssue,
    Series: {SeriesSno:0, Series_Name: "", VouTypeSno: this.globals.VTypRefiningIssue},
    Trans_No:"",
  ...
  ngOnInit(): void {    
    ...
    if (this.Transaction.TransSno === 0)
      ...
      this.SelectedDoc.VouTypeSno = this.globals.VTypRefiningIssue;
  ...
  SaveTransaction(){        
    ...
    this.progressService.sendUpdate("start","Saving Refining Issue");
    trans.saveTransaction().subscribe(data => {
      this.progressService.sendUpdate("stop","");
        if (data.queryStatus == 0) {
          ...
          this.GoBack();                  
          this.globals.SnackBar("info", this.Transaction.TransSno == 0 ? "Refining Issue Created successfully" : "Refining Issue updated successfully",2000);           
        }
    }, 
    error => {
      this.progressService.sendUpdate("stop","");
  ...
  DeleteTransaction(){
    ...
    this.globals.QuestionAlert("Are you sure you wanto to delete this Refining Issue?").subscribe(Response => {      
      if (Response == 1){
        ...
        itm.deleteTransaction().subscribe(data => {
          if (data.queryStatus == 0)
          {
            ...
            this.globals.SnackBar("info", "Refining Issue deleted successfully",1500);            
