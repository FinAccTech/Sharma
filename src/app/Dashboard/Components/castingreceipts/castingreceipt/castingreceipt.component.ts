  ...
  @AutoUnsubscribe
  @Component({
    selector: 'app-castingreceipt',
  templateUrl: './castingreceipt.component.html',
  styleUrls: ['./castingreceipt.component.scss']
  })
  
  export class CastingreceiptComponent implements OnInit {
    ...
    SelectedDoc: TypeDocHeader = {
      TransSno: 0,
      VouTypeSno: this.globals.VTypCastingReceipt,
      Series: {SeriesSno:0, Series_Name: "", VouTypeSno: this.globals.VTypCastingReceipt},
      Trans_No:"",
    ...
    ngOnInit(): void {    
      ...
      if (this.Transaction.TransSno === 0)
        ...
        this.SelectedDoc.VouTypeSno = this.globals.VTypCastingReceipt;
    ...
    SaveTransaction(){        
      ...
      this.progressService.sendUpdate("start","Saving Casting Receipt");
      trans.saveTransaction().subscribe(data => {
        this.progressService.sendUpdate("stop","");
          if (data.queryStatus == 0) {
            ...
            this.GoBack();                  
            this.globals.SnackBar("info", this.Transaction.TransSno == 0 ? "Casting Receipt Created successfully" : "Casting Receipt updated successfully",2000);           
          }
      }, 
      error => {
        this.progressService.sendUpdate("stop","");
    ...
    DeleteTransaction(){
      ...
      this.globals.QuestionAlert("Are you sure you wanto to delete this Casting Receipt?").subscribe(Response => {      
        if (Response == 1){
    ...
    getNewReference($event: TypeTransactions){
      ...
      if (this.SelectedDoc.Reference.TransSno && this.SelectedDoc.Reference.TransSno !== 0){    
        ...
        trans.getTransactions(this.SelectedDoc.Reference.TransSno, this.globals.VTypCastingIssue,0,0,0 ).subscribe(data => { 
