...
@AutoUnsubscribe
@Component({
  selector: 'app-jobworkdelivery',
  templateUrl: './jobworkdelivery.component.html',
  styleUrls: ['./jobworkdelivery.component.scss']
})

export class JobworkdeliveryComponent implements OnInit {
  ...
  SelectedDoc: TypeDocHeader = {
    TransSno: 0,
    VouTypeSno: this.globals.VTypJobworkDelivery,
    Series: {SeriesSno:0, Series_Name: "", VouTypeSno: this.globals.VTypJobworkDelivery},
    Trans_No:"",
  ...
  ngOnInit(): void {    
    ...
    if (this.Transaction.TransSno === 0)
      ...
      this.SelectedDoc.VouTypeSno = this.globals.VTypJobworkDelivery;
  ...
  SaveTransaction(){        
    ...
    this.progressService.sendUpdate("start","Saving Jobwork Delivery");
    trans.saveTransaction().subscribe(data => {
      this.progressService.sendUpdate("stop","");
        if (data.queryStatus == 0) {
          ...
          this.GoBack();                  
          this.globals.SnackBar("info", this.Transaction.TransSno == 0 ? "Jobwork Delivery Created successfully" : "Jobwork Delivery updated successfully",2000);           
        }
    }, 
    error => {
      this.progressService.sendUpdate("stop","");
  ...
  DeleteTransaction(){
    ...
    this.globals.QuestionAlert("Are you sure you wanto to delete this Jobwork Delivery?").subscribe(Response => {      
      if (Response == 1){
  ...
  getNewReference($event: TypeTransactions){
    if (this.SelectedDoc.Reference.TransSno && this.SelectedDoc.Reference.TransSno !== 0){    
      let trans = new ClsTransactions(this.dataService);
      this.progressService.sendUpdate("start","Getting Inward Details. Hold on");
      trans.getTransactions(this.SelectedDoc.Reference.TransSno, this.globals.VTypJobworkInward,0,0,0 ).subscribe(data => { 
