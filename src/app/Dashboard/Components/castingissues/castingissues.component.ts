  ...
  @Component({
    selector: 'app-castingissues',
  templateUrl: './castingissues.component.html',
  styleUrls: ['./castingissues.component.scss']
  }) 
  
  @AutoUnsubscribe
  export class CastingissuesComponent {
    ...
    ngOnInit(){
      this.LoadCastingIssueList(999,999);
    } 
  
    LoadCastingIssueList(FromDate: number, ToDate: number){
      let itm = new ClsTransactions(this.dataService);  
      this.progressService.sendUpdate("start","Loading Casting Issues...");  
      itm.getTransactions(0, this.globals.VTypCastingIssue,0,FromDate, ToDate).subscribe( data => {    
    ...
    FilterByDate(){
      this.LoadCastingIssueList(this.FromDate, this.ToDate) 
    }
    ...
    OpenTransaction(trans: TypeTransactions){            
      trans.VouTypeSno = this.globals.VTypCastingIssue;    
      this.router.navigate(['dashboard/castingissue', JSON.stringify(trans)]);
    } 
