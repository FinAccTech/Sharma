  ...
  @AutoUnsubscribe
  @Component({
    selector: 'app-castingissue',
    templateUrl: './castingissue.component.html',
    styleUrls: ['./castingissue.component.scss']
  })
  
  export class CastingissueComponent implements OnInit {
    ...
    SelectedDoc: TypeDocHeader = {
      TransSno: 0,
      VouTypeSno: this.globals.VTypCastingIssue,
      Series: {SeriesSno:0, Series_Name: "", VouTypeSno: this.globals.VTypCastingIssue},
      Trans_No:"",
    ...
    ngOnInit(): void {    
      ...
      if (this.Transaction.TransSno === 0)
        ...
        this.SelectedDoc.VouTypeSno = this.globals.VTypCastingIssue;
    ...
    SaveTransaction(){        
      ...
      this.progressService.sendUpdate("start","Saving Casting Issue");
      trans.saveTransaction().subscribe(data => {
        this.progressService.sendUpdate("stop","");
          if (data.queryStatus == 0) {
            ...
            this.GoBack();                  
            this.globals.SnackBar("info", this.Transaction.TransSno == 0 ? "Casting Issue Created successfully" : "Casting Issue updated successfully",2000);           
          }
      }, 
      error => {
        this.progressService.sendUpdate("stop","");
    ...
    DeleteTransaction(){
      ...
      this.globals.QuestionAlert("Are you sure you wanto to delete this Casting Issue?").subscribe(Response => {      
        if (Response == 1){
          ...
          itm.deleteTransaction().subscribe(data => {
            if (data.queryStatus == 0)
            {
              ...
              this.globals.SnackBar("info", "Casting Issue deleted successfully",1500);            
