...
export class SalesinvoiceComponent implements OnInit {
  ...
  DocConversion: boolean = false;

  SelectedDoc: TypeDocHeader = {
    TransSno: 0,
    VouTypeSno: this.globals.VTypSalesInvoice,
    Series: {SeriesSno:0, Series_Name: "", VouTypeSno: this.globals.VTypSalesInvoice},
    Trans_No:"",
  ...
  ngOnInit(): void {    
    if (this.Transaction.TransSno === 0)
      ...
      this.SelectedDoc.VouTypeSno = this.globals.VTypSalesInvoice;
  ...
  SaveTransaction(){        
    ...
    this.progressService.sendUpdate("start","Saving Sales Invoice");
  ...
  getDocRefrence($event: TypeTransactions){    
    this.DocConversion = false;
    if (this.SelectedDoc.Reference.TransSno && this.SelectedDoc.Reference.TransSno !== 0){    
      this.DocConversion = true; 
      let trans = new ClsTransactions(this.dataService);
      this.progressService.sendUpdate("start","Getting Delivery Detaisl. Hold on");
