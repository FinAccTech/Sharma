...
export class SalesinvoicesComponent {
  ...
  SalesInvoicesList!: TypeTransactions[];
  ...
  ngOnInit(){
    this.LoadSalesInvoiceList(999,999);
  } 

  LoadSalesInvoiceList(FromDate: number, ToDate: number){
    let itm = new ClsTransactions(this.dataService);    
    this.progressService.sendUpdate("start","Loading Sales Invoices...");
    itm.getTransactions(0, this.globals.VTypSalesInvoice,0, FromDate, ToDate).subscribe( data => {    
      this.progressService.sendUpdate("stop","");      
      if (data.queryStatus == 0){
        this.globals.ShowAlert(this.globals.DialogTypeError, data.apiData);      
      }
      else{              
        this.SalesInvoicesList = JSON.parse(data.apiData);                
  ...
  FilterByDate(){
    this.LoadSalesInvoiceList(this.FromDate, this.ToDate) 
  }
  ...
  OpenTransaction(trans: TypeTransactions){            
    trans.VouTypeSno = this.globals.VTypSalesInvoice;    
    this.router.navigate(['dashboard/salesinvoice', JSON.stringify(trans)]);
  } 
      ...
      if (Response == 1){
        ...
        grp.deleteTransaction().subscribe(data => {
          if (data.queryStatus == 0)
          {
            ...
            const index =  this.SalesInvoicesList.indexOf(trans);
            this.SalesInvoicesList.splice(index,1);
            this.LoadDataIntoMatTable();
          }
        })        
      }
  ...
  LoadDataIntoMatTable(){
    this.dataSource = new MatTableDataSource<TypeTransactions> (this.SalesInvoicesList);        
