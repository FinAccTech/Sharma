...
export class PurchaseordersComponent {
  ...
  PurchaseOrdersList!: TypeTransactions[];
  ...
  ngOnInit(){
    this.LoadPurchaseOrderList(999,999);
  } 

  LoadPurchaseOrderList(FromDate: number, ToDate: number){
    let itm = new ClsTransactions(this.dataService);    
    this.progressService.sendUpdate("start","Loading Purchase Orders...");
    itm.getTransactions(0, this.globals.VTypPurchaseOrder,0, FromDate, ToDate).subscribe( data => {       
      this.progressService.sendUpdate("stop","");   
      if (data.queryStatus == 0){
        this.globals.ShowAlert(this.globals.DialogTypeError, data.apiData);      
      }
      else{              
        this.PurchaseOrdersList = JSON.parse(data.apiData);                
  ...
  FilterByDate(){
    this.LoadPurchaseOrderList(this.FromDate, this.ToDate) 
  }
  ...
  OpenTransaction(trans: TypeTransactions){            
    trans.VouTypeSno = this.globals.VTypPurchaseOrder;        
    this.router.navigate(['dashboard/purchaseorder', JSON.stringify(trans)]);
  } 
      ...
      if (Response == 1){
        ...
        grp.deleteTransaction().subscribe(data => {
          if (data.queryStatus == 0)
          {
            ...
            this.globals.SnackBar("info","Purchase Order deleted successfully",1500);
            const index =  this.PurchaseOrdersList.indexOf(trans);
            this.PurchaseOrdersList.splice(index,1);
            this.LoadDataIntoMatTable();
          }
        })        
      }
  ...
  LoadDataIntoMatTable(){
    this.dataSource = new MatTableDataSource<TypeTransactions> (this.PurchaseOrdersList);        
