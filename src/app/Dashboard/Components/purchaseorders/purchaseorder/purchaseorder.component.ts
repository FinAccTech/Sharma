...
export class PurchaseorderComponent implements OnInit {
  ...
  SelectedDoc: TypeDocHeader = {
    TransSno: 0,
    VouTypeSno: this.globals.VTypPurchaseOrder,
    Series: {SeriesSno:0, Series_Name: "", VouTypeSno: this.globals.VTypPurchaseOrder},
    Trans_No:"",
  ...
  ngOnInit(): void {    
    ...
    if (this.Transaction.TransSno === 0)
      ...
      this.SelectedDoc.VouTypeSno = this.globals.VTypPurchaseOrder;
  ...
  SaveTransaction(){       
    ...
    this.progressService.sendUpdate("start","Saving Purchase Order");
    trans.saveTransaction().subscribe(data => {
      this.progressService.sendUpdate("stop","");
        if (data.queryStatus == 0) {
          ...
          this.globals.SnackBar("info", this.Transaction.TransSno == 0 ? "Purchase Order Created successfully" : "Purchase Order updated successfully",2000);           
  ...
  DeleteTransaction(){
    ...
    this.globals.QuestionAlert("Are you sure you wanto to delete this Purchase Order?").subscribe(Response => {      
      if (Response == 1){
