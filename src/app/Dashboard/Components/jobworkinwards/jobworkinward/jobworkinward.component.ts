...
@Component({
  selector: 'app-jobworkinward',
  templateUrl: './jobworkinward.component.html',
  styleUrls: ['./jobworkinward.component.scss']
})
...
export class JobworkinwardComponent implements OnInit {
  ...
  SelectedDoc: TypeDocHeader = {
    TransSno: 0,
    VouTypeSno: this.globals.VTypJobworkInward,
    Series: {SeriesSno:0, Series_Name: "", VouTypeSno: this.globals.VTypJobworkInward},
    Trans_No:"",
  ...
  ngOnInit(): void {    
    ...
    if (this.Transaction.TransSno === 0)
      ...
      this.SelectedDoc.VouTypeSno = this.globals.VTypJobworkInward;
  ...
  SaveTransaction(){       
    ...
    this.progressService.sendUpdate("start","Saving Jobwork Inward");
    trans.saveTransaction().subscribe(data => {
      this.progressService.sendUpdate("stop","");
        if (data.queryStatus == 0) {
          ...
          this.globals.SnackBar("info", this.Transaction.TransSno == 0 ? "Jobwork Inward Created successfully" : "Jobwork Inward updated successfully",2000);           
  ...
  DeleteTransaction(){
    if (this.Transaction.TransSno == 0){
      this.globals.SnackBar("error", "No Inward selected to delete",1200);
      return;
    }
    this.globals.QuestionAlert("Are you sure you wanto to delete this Jobwork Inward?").subscribe(Response => {      
      if (Response == 1){
