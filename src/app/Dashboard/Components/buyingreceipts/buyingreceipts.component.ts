...
@Component({
  selector: 'app-buyingreceipts',
  templateUrl: './buyingreceipts.component.html',
  styleUrls: ['./buyingreceipts.component.scss']
}) 

@AutoUnsubscribe
export class BuyingreceiptsComponent {
  ...
  BuyingReceiptsList!: TypeTransactions[];
  ...
  ngOnInit(){
    this.LoadBuyingReceiptList(999,999);
  } 

  LoadBuyingReceiptList(FromDate: number, ToDate: number){
    let itm = new ClsTransactions(this.dataService);    
    this.progressService.sendUpdate("start","Loading Buying Receipts...");
    itm.getTransactions(0, this.globals.VTypBuyingReceipt,0, FromDate, ToDate).subscribe( data => {        
      this.progressService.sendUpdate("stop","");  
      if (data.queryStatus == 0){
        this.globals.ShowAlert(this.globals.DialogTypeError, data.apiData);      
      }
      else{              
        this.BuyingReceiptsList = JSON.parse(data.apiData);                
  ...
  FilterByDate(){
    this.LoadBuyingReceiptList(this.FromDate, this.ToDate) 
  }
  ...
  OpenTransaction(trans: TypeTransactions){            
    trans.VouTypeSno = this.globals.VTypBuyingReceipt;    
    this.router.navigate(['dashboard/buyingreceipt', JSON.stringify(trans)]);
  } 
      ...
      if (Response == 1){
        ...
        grp.deleteTransaction().subscribe(data => {
          if (data.queryStatus == 0)
          {
            ...
            const index =  this.BuyingReceiptsList.indexOf(trans);
            this.BuyingReceiptsList.splice(index,1);
            this.LoadDataIntoMatTable();
          }
        })        
      }
  ...
  LoadDataIntoMatTable(){
    this.dataSource = new MatTableDataSource<TypeTransactions> (this.BuyingReceiptsList);        
