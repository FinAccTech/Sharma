...
@AutoUnsubscribe
@Component({
  selector: 'app-smeltingreceipts',
  templateUrl: './smeltingreceipts.component.html',
  styleUrls: ['./smeltingreceipts.component.scss']
}) 

export class SmeltingreceiptsComponent {
  ...
  SmeltingReceiptsList!: TypeTransactions[];
  ...
  ngOnInit(){
    this.LoadSmeltingReceiptList(999,999);
  } 

  LoadSmeltingReceiptList(FromDate: number, ToDate: number){
    let itm = new ClsTransactions(this.dataService);    
    this.progressService.sendUpdate("start","Loading Smelting Receipts...");
    itm.getTransactions(0, this.globals.VTypSmeltingReceipt,0, FromDate, ToDate).subscribe( data => {          
      this.progressService.sendUpdate("stop","");
      if (data.queryStatus == 0){
        this.globals.ShowAlert(this.globals.DialogTypeError, data.apiData);      
      }
      else{              
        this.SmeltingReceiptsList = JSON.parse(data.apiData);                
  ...
  FilterByDate(){
    this.LoadSmeltingReceiptList(this.FromDate, this.ToDate) 
  }
  ...
  OpenTransaction(trans: TypeTransactions){            
    trans.VouTypeSno = this.globals.VTypSmeltingReceipt;    
    this.router.navigate(['dashboard/smeltingreceipt', JSON.stringify(trans)]);
  } 
      ...
      if (Response == 1){
        ...
        grp.deleteTransaction().subscribe(data => {
          if (data.queryStatus == 0)
          {
            ...
            const index =  this.SmeltingReceiptsList.indexOf(trans);
            this.SmeltingReceiptsList.splice(index,1);
            this.LoadDataIntoMatTable();
          }
        })        
      }
  ...
  LoadDataIntoMatTable(){
    this.dataSource = new MatTableDataSource<TypeTransactions> (this.SmeltingReceiptsList);        
