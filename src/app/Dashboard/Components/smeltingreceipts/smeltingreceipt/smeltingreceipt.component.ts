...
@AutoUnsubscribe
@Component({
  selector: 'app-smeltingreceipt',
  templateUrl: './smeltingreceipt.component.html',
  styleUrls: ['./smeltingreceipt.component.scss']
})

export class SmeltingreceiptComponent implements OnInit {
  ...
  SelectedDoc: TypeDocHeader = {
    TransSno: 0,
    VouTypeSno: this.globals.VTypSmeltingReceipt,
    Series: {SeriesSno:0, Series_Name: "", VouTypeSno: this.globals.VTypSmeltingReceipt},
    Trans_No:"",
  ...
  ngOnInit(): void {    
    ...
    if (this.Transaction.TransSno === 0)
      ...
      this.SelectedDoc.VouTypeSno = this.globals.VTypSmeltingReceipt;
  ...
  SaveTransaction(){        
    ...
    this.progressService.sendUpdate("start","Saving Smelting Receipt");
    trans.saveTransaction().subscribe(data => {
      this.progressService.sendUpdate("stop","");
        if (data.queryStatus == 0) {
          ...
          this.GoBack();                  
          this.globals.SnackBar("info", this.Transaction.TransSno == 0 ? "Smelting Receipt Created successfully" : "Smelting Receipt updated successfully",2000);           
        }
    }, 
    error => {
      this.progressService.sendUpdate("stop","");
  ...
  DeleteTransaction(){
    ...
    this.globals.QuestionAlert("Are you sure you wanto to delete this Smelting Receipt?").subscribe(Response => {      
      if (Response == 1){
        ...
        itm.deleteTransaction().subscribe(data => {
          if (data.queryStatus == 0)
          {
            ...
            this.globals.SnackBar("info", "Smelting Receipt deleted successfully",1500);            
  ...
  getNewReference($event: TypeTransactions){
    if (this.SelectedDoc.Reference.TransSno && this.SelectedDoc.Reference.TransSno !== 0){    
      let trans = new ClsTransactions(this.dataService);
      this.progressService.sendUpdate("start","Getting Issue Detaisl. Hold on");
      trans.getTransactions(this.SelectedDoc.Reference.TransSno, this.globals.VTypSmeltingIssue,0,0,0 ).subscribe(data => {     
        this.progressService.sendUpdate("stop","");               
        this.Transaction.Party = JSON.parse (JSON.parse (data.apiData)[0].Party_Json)[0];

        this.GridList = [
          {
            "Item": { "ItemSno": this.SelectedDoc.Reference.CompSno==1 ?  1 : 4, "Item_Code": "I1", "Item_Name": "AU", "Name": "AU", "Details": "Code:" + "I1" }, "Karat": 24, "PurityPer": 100, "Remarks": "Melted Bar", "Qty": 1,
            "GrossWt": this.SelectedDoc.Reference.TotGrossWt!, "NettWt": this.SelectedDoc.Reference.TotGrossWt!, "Wastage": 0, "SilverWt":0, "SilverPurity":0, 
          ...
          {
              "Item": { "ItemSno": this.SelectedDoc.Reference.CompSno==1 ?  3 : 6, "Item_Code": "I3", "Item_Name": "Sample Item", "Name": "Sample Item", "Details": "Code:" + "I3" }, "Karat": 24, "PurityPer": 100, "Remarks": "Sample Item", "Qty": 1,
              ...
              Batch_No: '',
              StoneWt: 0,
              BatchItems: this.SelectedDoc.Trans_No + "/SMP1"
          }
      ]
      },       
      error => { 
